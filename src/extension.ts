...
async function tryGenerateFromIDE(repo: GitRepository, availableCommands: string[]): Promise<string | undefined> {
	const commands = [
		...
		{ id: 'codeium.generateCommitMessage', name: 'Codeium' },
		{ id: 'codiumai.generateCommitMessage', name: 'Codium' },
		{ id: 'qodo.generateCommitMessage', name: 'Qodo' },
	...
	for (const cmd of commands) {
		if (availableCommands.includes(cmd.id)) {
			...
			try {
				// Some commands (e.g. github.copilot.git.generateCommitMessage) are UI-context commands
				// that may not accept a repo argument. Try with repo first, then without.
				let result = await vscode.commands.executeCommand(cmd.id, repo);
				...
				// 2. Poll the input box — AI-based generators (e.g. Copilot) write asynchronously
				// and can take several seconds. Poll every 500ms for up to 30 seconds.
				...
				while (elapsed < pollMaxMs) {
					...
					if (repo.inputBox.value && repo.inputBox.value !== beforeValue) {
						console.log(`Autocommiter: Successfully generated message using ${cmd.name} (updated input box after ${elapsed}ms)`);
						return repo.inputBox.value.trim();
					}
				}

				// 3. If passing repo didn't work, retry the command without arguments
				//    (some UI-context commands ignore unknown argument types)
				console.log(`Autocommiter: Retrying ${cmd.name} without repo argument`);
				const beforeValue2 = repo.inputBox.value;
				result = await vscode.commands.executeCommand(cmd.id);
				if (result && typeof result === 'string' && result.trim().length > 0) {
					console.log(`Autocommiter: Successfully generated message using ${cmd.name} (no-arg, returned string)`);
					return result.trim();
				}
				elapsed = 0;
				while (elapsed < 3000) {
					await new Promise(resolve => setTimeout(resolve, pollIntervalMs));
					elapsed += pollIntervalMs;
					if (repo.inputBox.value && repo.inputBox.value !== beforeValue2) {
						console.log(`Autocommiter: Successfully generated message using ${cmd.name} (no-arg, updated input box after ${elapsed}ms)`);
						return repo.inputBox.value.trim();
					}
				}
		...
		vscode.window.withProgress({
			...
			for (const repo of repos) {
				...
				try {
					...
					// Force refresh of Git state so IDE generators see the changes
					try {
						if (typeof repo.status === 'function') {
							await repo.status();
						}
					} catch (e) {
						console.error(`[${repoName}] Failed to refresh repo status`, e);
					}
					...
					// 6) Final check
					if (!message) {
						const errorMsg = `Generation failed for ${repoName}. (Tried: Copilot/IDE, CLI, and Fallback API). Please check your "gh auth login" or API key if using the fallback.`;
						console.error(`Autocommiter: ${errorMsg}`);
						results.push({ name: repoName, success: false, error: 'All generation methods failed' });
						continue;
					}
